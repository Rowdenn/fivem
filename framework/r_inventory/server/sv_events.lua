...
AddEventHandler('r_inventory:getPlayerInventory', function()
    ...
    local result = Query('SELECT slot, item, count, metadata FROM inventories WHERE identifier = ?',
        { identifier })
...
AddEventHandler('r_inventory:getGroundItems', function()
    ...
    local result = Query(
        'SELECT id, item, count, x, y, z FROM ground_items WHERE SQRT(POW(x - ?, 2) + POW(y - ?, 2) + POW(z - ?, 2)) <= 2.0',
        {
            coords.x,
            coords.y,
            coords.z
...
AddEventHandler('r_inventory:updateGroundItemCoords', function(itemId, newCoords)
    ...
    if distance <= 50.0 then -- Seuil de sécurité
        Execute('UPDATE ground_items SET x = @x, y = @y, z = @z WHERE id = @id', {
            ['@x'] = newCoords.x,
            ['@y'] = newCoords.y,
            ['@z'] = newCoords.z,
            ['@id'] = itemId
...
AddEventHandler('r_inventory:getAllGroundItems', function()
    ...
    local result = Query(
        'SELECT id, item, count, x, y, z FROM ground_items WHERE SQRT(POW(x - ?, 2) + POW(y - ?, 2) + POW(z - ?, 2)) <= 10.0',
        {
            coords.x,
            coords.y,
            coords.z
...
AddEventHandler('r_inventory:dropItem', function(itemName, count, slot)
    ...
    Execute('INSERT INTO ground_items (item, count, x, y, z) VALUES (@item, @count, @x, @y, @z)', {
        ['@item'] = itemName,
        ['@count'] = count,
        ['@x'] = coords.x,
        ['@y'] = coords.y,
    ...
    local newItem = Query(
        'SELECT id, item, count, x, y, z FROM ground_items WHERE x = @x AND y = @y AND z = @z ORDER BY id DESC LIMIT 1',
        {
            ['@x'] = coords.x,
            ['@y'] = coords.y,
            ['@z'] = coords.z
...
AddEventHandler('r_inventory:pickupItem', function(groundItemId, toSlot)
    ...
    local groundItem = Query('SELECT * FROM ground_items WHERE id = @id', {
        ['@id'] = groundItemId
    })

    if groundItem[1] then
        -- Vérifier que le slot de destination est libre
        local existingItem = Query('SELECT * FROM inventories WHERE identifier = @identifier AND slot = @slot',
            {
        ...
        if not existingItem[1] then
            ...
            Execute('DELETE FROM ground_items WHERE id = @id', {
                ['@id'] = groundItemId
            })
...
AddEventHandler('r_inventory:useQuickSlot', function(quickSlot)
    ...
    local result = Query('SELECT * FROM inventories WHERE identifier = @identifier AND slot = @slot', {
        ['@identifier'] = identifier,
        ['@slot'] = quickSlot
    })
...
AddEventHandler('r_inventory:moveItem', function(fromSlot, fromType, toSlot, toType)
    ...
    if fromType == 'player' and toType == 'player' then
        ...
        local item1 = Query('SELECT * FROM inventories WHERE identifier = @identifier AND slot = @slot', {
            ['@identifier'] = identifier,
            ['@slot'] = fromSlot
        })

        local item2 = Query('SELECT * FROM inventories WHERE identifier = @identifier AND slot = @slot', {
            ['@identifier'] = identifier,
            ['@slot'] = toSlot
        })
        ...
        if item1[1] then
            ...
            Execute('UPDATE inventories SET slot = @slot WHERE identifier = @identifier AND slot = @fromSlot', {
                ['@slot'] = toSlot,
                ['@identifier'] = identifier,
                ['@fromSlot'] = fromSlot
            })
            ...
            if item2[1] then
                Execute(
                    'UPDATE inventories SET slot = @slot WHERE identifier = @identifier AND slot = @toSlot AND id != @excludeId',
                    {
                        ['@slot'] = fromSlot,
                        ['@identifier'] = identifier,
                        ['@toSlot'] = toSlot,
